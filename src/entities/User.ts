...
export class User extends DefaultEntity {
  ...
  @Column({ length: 64, nullable: true }) password?: string | null;

  // UI에 표시할 이름, 사용자끼리 식별시 사용
  @Column({ length: 16, nullable: true }) nickname?: string | null;

  // 알림을 위한 컬럼, 서버가 파이어베이스에 메시지를 요청할 때 사용
  @Column({ length: 163, nullable: true, unique: true }) notificationToken?: string | null;

  // 소셜 로그인을 위한 컬럼, 회원가입 시 닉네임 입력전에 뒤로가기 했을 경우를 위해 저장 필요, 일반 로그인은 없다.
  @Column({ nullable: true }) oAuthUID?: string | null;
  ...
  @ManyToMany(() => Todo, { cascade: true }) @JoinTable() todos: Todo[];
}
